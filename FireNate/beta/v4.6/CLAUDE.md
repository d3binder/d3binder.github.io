...
Four themes: `light` (default, no `data-theme` attribute — this preserves
old CSS written before other themes existed), `dark`, `neon`, `sunshine`.
`assets/js/theme.js` exposes `window.FNTheme = { get, set, toggle, THEMES }`
and sets `data-theme` on `<html>`; `theme-init.js` applies the saved theme
before first paint (load it synchronously, first thing in `<head>`, to
